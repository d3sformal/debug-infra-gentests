...
To correctly handle program crashes of the instrumented application, you need to run the [`ipc-finalizer`](../ipc-finalizer/) with correct arguments (number of buffers) to indicate the end of call tracing or argument capture (**after the target application crashes**):

    ../ipc-finalizer/ipc-fin /llcap-capture-base-semfull <number_of_buffers>

Default number of buffers can be found in [llcap-server's](../llcap-server/) `--help` message:
...
This step generates the `module-maps` directory and compiles the exmaple binary ([main.cpp](./main.cpp)).

Then, open another terminal in the [../llcap-server](../llcap-server/) directory. Here, we will run the `llcap-server`:

(note: if `llcap-server` fails early during its initialization, you can use the `--cleanup` flag to perform cleanup of unreleased resources)

    cargo r  --release -- --modmap ../example-arg-replacement/module-maps/ trace-calls

The serveri is now `listening` - waiting for our target application to be started.

**Back in this (`example-arg-replacement`) directory**, run the example application:

    ./arg-replacement

On the **`llcap-server`**'s side, you should see similar output, prompting you to perform selection:
...
You should select the indicies from the first column to indicate which functions you'd like to perform testing on. For primitive type replacement, input `1`, for *custom* type replacement, input `0`. Next, press enter twice, the (important) result of this stage will be saved in the default location (e.g. `./selected-fns.bin`).
...
Next, run `llcap-server`, passing the genrated files to it, as well as the command line invocation of the instrumented binary (as tail argunment pack):
    ...
    cargo r --release -- --modmap ../example-arg-replacement/module-maps/ capture-args -s ./selected-fns.bin -o ./arg-traces-dir ../example-arg-replacement/arg-replacement
...
**Alternatively**, you can prepend the command with the removal of the capture output directory as **the directory is expected not to exist** by the `llcap-server`.
    ...
    rm -rf ./arg-traces-dir && cargo r --release -- --modmap ../example-arg-replacement/module-maps/ capture-args -s ./selected-fns.bin -o ./arg-traces-dir ../example-arg-replacement/arg-replacement
...
We are now ready to execute tests based on the recorded data. **No recompilation is needed for this stage**. We create output directory and run `llcap-server`:
    ...
    cargo r --release -- --modmap ../example-arg-replacement/module-maps/ test -s ./selected-fns.bin -c ./arg-traces-dir -t ./arg-trace-outputs/ ../example-arg-replacement/arg-replacement
