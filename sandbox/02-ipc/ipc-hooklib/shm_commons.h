...
typedef struct {
  ...
  uint32_t mode; // required for argument capture and testing, 0 for call tracing, 1 for capture, 2 for testing
  ...
  // false if zero, indicates whether we are inside a forked process - and should prevent further forking when instrumented code (preamble) is reached multiple times
  uint32_t forked;
  // number of arguments to read, should prevent argument hijacking when instrumented code is reached multiple times (decrement & check if zero) 
  uint32_t arg_count;
