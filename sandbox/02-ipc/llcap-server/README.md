...
You can obtain `cargo` and `rustc` from your OS's package distribution system (package `cargo` should provide both) or by installing via `rustup`. 

`rustup` is preffered as it provides the latest version of the toolchain.
...
* `*.rs` files - top-level drivers, constants, logging, CLI argument declaration
  * `main.rs`
    * each `match` arm performs a different stage; the arms are perfectly independent of one another
    * albeit completely independent, one can argue they bloat the `main` function, therefore it is subject to a future refactor
...
### Rust code reading quickstart (skip if familiar with Rust, anyhow crate)

#### Returning + expanded expressions

You can omit the `return` keyword in the last line of the function. You must also omit the semicolon in that case:

```rust
fn add(x: u16, y: u16) -> u16 {
  x + y // equivalent to return x + y;
}
```

Other structures are also expressions whose value is also influenced by omitting the semicolon at the end of the last line, for example:
...
fn main() {
  ...
  let y = {
    let z = 23;
    // you can also call functions here
    x + z // no ; here!
 }; // y = x + 23 
  let z = if y < 33 {
    "foo" // no ; here!
...
For more information, consult [the Rust book - statements & expressions](https://doc.rust-lang.org/stable/book/ch03-03-how-functions-work.html#statements-and-expressions)
...
```rust
// this is just a demonstration, combining the above with the Result usage
// assume all functions but `other` return Result<u16, String>
fn foo() -> Result<u16, String> {
  let x = function_that_may_fail_with_err()?; 
  // x is u16 (due to the usage of ?)
  let y = other(x);
  let z = bar(z)?; // if any one of those ? lines are Err, the Err is returned, and the subsequent statements do not affect the program
  let z = {
    operation_1(y)?;
    operation_2(y)?;
    if something {
...
For more information, consult [the Rust book - Result](https://doc.rust-lang.org/stable/book/ch09-02-recoverable-errors-with-result.html)

`llcap-server` uses the [`anyhow`](https://docs.rs/anyhow/latest/anyhow/) crate which simplifies the Rust's `Result<T, E>` type interactions.

When you see a return type of `Result<T>` (without the error type), you see the `anyhow` crate's `Result` used, and you may also see `bail!(expr)` and `ensure!` macros around. 

`bail!` constructs the `Err(expr)` variant of the `Result` and early-returns it - read it as an early return. 

`ensure!(cond, message...)` macro *ensures* `cond` is true. If `cond` is false, the code generated by `ensure!` early-returns the `Err` variant with the specified `message...` (a string format and the arguments to be formatted). This allows the following constructions:
...
### Notable code (relative to `src`)

* thread-monitoring `spawn_process_monitor` in [stages/common.rs](./src/stages/common.rs)
* the `CaptureLoop` trait in [shmem_capture](./src/shmem_capture/mod.rs)
  * represents the default modus operandi of "capturing" stages - reading from the shared memory buffer (call tracing and argument capture) and transforming the data
  * [call tracing](./src/shmem_capture/call_tracing.rs) and [argument capture](./src/shmem_capture/arg_capture.rs) each implement this trait (search `CaptureLoop for`)
  * the trait itself defines the `run` function that ties its interface together
* `fn send_metadata` sending the communication parameters (in [`shmem_capture`](./src/shmem_capture/mod.rs))
* various "safer" wrappers around raw (shared) memory buffers in [`shmem_capture`](./src/shmem_capture/mod.rs): `BorrowedOneshotWritePtr`, `BorrowedReadBuffer`, `ReadOnlyBufferPtr`, `BufferStartPtr`

#### Call tracing
* simple capture loop of the call tracing stage in [`shmem_capture/call_tracing.rs`](./src/shmem_capture/call_tracing.rs) - `update_from_buffer`

#### Argument capture
* [`ArgSizeTypeRef` enum](./src/sizetype_handlers.rs) identical with the `llvm-pass`'s [`LlcapSizeType`](../../01-llvm-ir/llvm-pass/src/typeids.h)
* [`PartialCaptureState`](./src/shmem_capture/arg_capture.rs) implementing the reader side of the argument capture
* argument packet writer [`struct FunctionPacketDumper`](./src/stages/arg_capture.rs)

#### Testing
* `test_job` launching in [`main.rs`](./src/main.rs) and implementation in [`testing.rs`](./src/stages/testing.rs) (spawning of the tests)
* `test_server_job` and `test_coordinator_case_handler` in [`testing.rs`](./src/stages/testing.rs) implementing the `llcap-server`'s side of the testing-related communication (packets, test status, ...)
* argument packet readers [`struct CaptureReader`](./src/stages/arg_capture.rs)
