...
#ifndef VSTR_ASTINJ_UTIL_HPP
#define VSTR_ASTINJ_UTIL_HPP
...
template <class ID, class KEY> class CFunctionIdGen {
public:
...
public:
  ...
  std::vector<std::pair<KEY, ID>> fetchFunctionIdMapping() const {
    return std::vector<std::pair<KEY, ID>>(m_mapping.cbegin(),
                                           m_mapping.cend());
  }
...
template <class T>
inline bool dumpLines(const T &iterable_outstreamable,
